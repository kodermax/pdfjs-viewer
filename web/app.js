...
function configure(PDFJS) {
  ...
  if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('FIREFOX || MOZCENTRAL || GENERIC || CHROME')) {
...
let PDFViewerApplication = {
  ...
  initialize(appConfig) {
    ...
    return this
      ...
      .then(() => {
        // Bind the various event handlers *after* the viewer has been initialized, to
        // prevent errors if an event arrives too soon.
        this.bindEvents();
        ...
        this
          ...
          .then(() => {
            // Dispatch the 'localized' event on the `eventBus` once the viewer has been
            // fully initialized and translated.
            this
              .eventBus
              .dispatch('localized');
          });
        ...
        if (this.isViewerEmbedded && !PDFJS.isExternalLinkTargetSet()) {
          // Prevent external links from "replacing" the viewer, when it's embedded in
          // e.g. an iframe or an object.
  ...
  _parseHashParameters() {
    ...
    if (typeof PDFJSDev === 'undefined' || !PDFJSDev.test('PRODUCTION') || viewerPrefs['pdfBugEnabled']) {
      ...
      if ('textlayer' in hashParams) {
        switch (hashParams['textlayer']) {
          ...
          case 'hover':
            ...
            viewer
              .classList
              .add('textLayer-' + hashParams['textlayer']);
            break;
        }
      }
      ...
      // Locale can be changed only when special debugging flags is present in the
      // hash section of the URL, or development version of viewer is used. It is not
      // possible to change locale for Firefox extension builds.
      if ((typeof PDFJSDev === 'undefined' || !PDFJSDev.test('PRODUCTION') || !PDFJSDev.test('FIREFOX || MOZCENTRAL')) && 'locale' in hashParams) {
  ...
  _initializeViewerComponents() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pdfThumbnailViewer = new PDFThumbnailViewer({container: thumbnailContainer, renderingQueue: pdfRenderingQueue, linkService: pdfLinkService, l10n: this.l10n});
      pdfRenderingQueue.setThumbnailViewer(this.pdfThumbnailViewer);

      this.pdfHistory = new PDFHistory({linkService: pdfLinkService, eventBus});
      ...
      this.findController.onUpdateState = (state, previous, matchCount) => {
        if (this.supportsIntegratedFind) {
          this
            .externalServices
            .updateFindControlState({result: state, findPrevious: previous});
        } else {
          this
            .findBar
      ...
      this
        .pdfViewer
        .setFindController(this.findController);
      ...
      this.pdfDocumentProperties = new PDFDocumentProperties(appConfig.documentProperties, this.overlayManager, this.l10n);

      this.pdfCursorTools = new PDFCursorTools({container, eventBus, preferences: this.preferences});
      ...
      if (this.supportsFullscreen) {
        this.pdfPresentationMode = new PDFPresentationMode({container, viewer, pdfViewer: this.pdfViewer, eventBus, contextMenuItems: appConfig.fullscreen});
      }

      this.passwordPrompt = new PasswordPrompt(appConfig.passwordOverlay, this.overlayManager, this.l10n);

      this.pdfOutlineViewer = new PDFOutlineViewer({container: appConfig.sidebar.outlineView, eventBus, linkService: pdfLinkService});

      this.pdfAttachmentViewer = new PDFAttachmentViewer({container: appConfig.sidebar.attachmentsView, eventBus, downloadManager});
      ...
      this.pdfSidebar.onToggled = this
        .forceRendering
        .bind(this);
  ...
  setTitleUsingUrl(url) {
    ...
    if (!title) {
      try {
        ...
        // decodeURIComponent may throw URIError, fall back to using the unprocessed url
        // in that case
        title = url;
      }
    }
  ...
  open(file, args) {
    ...
    if (typeof PDFJSDev === 'undefined' || !PDFJSDev.test('PRODUCTION')) {
      parameters.docBaseUrl = document
        .URL
        .split('#')[0];
    ...
    loadingTask.onUnsupportedFeature = this
      .fallback
      .bind(this);
    ...
    return loadingTask
      ...
      .then((pdfDocument) => {
        ...
        if (exception instanceof InvalidPDFException) {
          ...
          loadingErrorMessage = this
            .l10n
            .get('invalid_file_error', null, 'Invalid or corrupted PDF file.');
        } else if (exception instanceof MissingPDFException) {
          // special message for missing PDF's
          loadingErrorMessage = this
            .l10n
            .get('missing_file_error', null, 'Missing PDF file.');
        } else if (exception instanceof UnexpectedResponseException) {
          loadingErrorMessage = this
            .l10n
            .get('unexpected_response_error', null, 'Unexpected server response.');
        } else {
          loadingErrorMessage = this
            .l10n
            .get('loading_error', null, 'An error occurred while loading the PDF.');
  ...
  download() {
    ...
    // Use this.url instead of this.baseUrl to perform filename detection based on
    // the reference fragment as ultimate fallback if needed.
    let filename = this.contentDispositionFilename || getPDFFileNameFromURL(this.url);
    let downloadManager = this.downloadManager;
    downloadManager.onerror = (err) => {
      // This error won't really be helpful because it's likely the fallback won't
      // work either (or is already open).
  ...
  fallback(featureId) {
    if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('FIREFOX || MOZCENTRAL')) {
      // Only trigger the fallback once so we don't spam the user with messages for
      // one PDF.
      if (this.fellback) {
        return;
      }
  ...
  progress(level) {
    ...
    // When we transition from full request to range requests, it's possible that we
    // discard some of the loaded data. This can cause the loading bar to move
    // backwards. So prevent this by only updating the bar if it increases.
    if (percent > this.loadingBar.percent || isNaN(percent)) {
      this.loadingBar.percent = percent;

      // When disableAutoFetch is enabled, it's not uncommon for the entire file to
      // never be fetched (depends on e.g. the file structure). In this case the
      // loading bar will not be completely filled, nor will it be hidden. To prevent
      // displaying a partially filled loading bar permanently, we hide it when no
      // data has been loaded during a certain amount of time.
  ...
  load(pdfDocument) {
    ...
    pdfDocument
      ...
      .then(() => {
        ...
        firstPagePromise.then(() => {
          this
            .eventBus
            .dispatch('documentload', {source: this});
        });
      });

    // Since the `setInitialView` call below depends on this being resolved, fetch
    // it early to avoid delaying initial rendering of the PDF document.
    let pageModePromise = pdfDocument
      .getPageMode()
      .catch(function () {/* Avoid breaking initial rendering; ignoring errors. */
      });

    this
      .toolbar
      .setPagesCount(pdfDocument.numPages, false);
    this
      .secondaryToolbar
    ...
    if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
      ...
      baseDocumentUrl = location
        .href
        .split('#')[0];
    }
    this
      .pdfLinkService
      .setDocument(pdfDocument, baseDocumentUrl);
    this
      .pdfDocumentProperties
      .setDocument(pdfDocument, this.url);
    ...
    firstPagePromise.then((pdfPage) => {
      this
        .loadingBar
        .setWidth(this.appConfig.viewerContainer);

      if (!PDFJS.disableHistory && !this.isViewerEmbedded) {
        // The browsing history is only enabled when the viewer is standalone, i.e. not
        // when it is embedded in a web page.
        let resetHistory = !this.viewerPrefs['showPreviousViewOnLoad'];
        this
          .pdfHistory
          .initialize(id, resetHistory);
      ...
      Promise
        .all([storePromise, pageModePromise])
        .then(([
          values = {},
          pageMode
        ]) => {
          ...
          return {hash, rotation, sidebarView};
        })
        .then(({hash, rotation, sidebarView}) => {
          ...
          // Make all navigation keys work on document load, unless the viewer is embedded
          // in a web page.
          if (!this.isViewerEmbedded) {
            pdfViewer.focus();
          }
        ...
        .then(() => {
          // For documents with different page sizes, once all pages are resolved, ensure
          // that the correct location becomes visible on load.
        ...
        .then(function () {
          // At this point, rendering of the initial page(s) should always have started
          // (and may even have completed). To prevent any future issues, e.g. the
          // document being completely blank on load, always trigger rendering here.
          pdfViewer.update();
        });
    });
    ...
    pdfDocument
      ...
      .then((labels) => {
        ...
        // Changing toolbar page display to use labels and we need to set the label of
        // the current page.
        this
          .toolbar
          .setPagesCount(pdfDocument.numPages, true);
        this
          .toolbar
          .setPageNumber(pdfViewer.currentPageNumber, pdfViewer.currentPageLabel);
      });
    ...
    Promise
      .all([onePageRendered, animationStarted])
      .then(() => {
        pdfDocument
          .getOutline()
          .then((outline) => {
    ...
    pdfDocument
      ...
      .then(({info, metadata, contentDispositionFilename}) => {
        ...
        console.log('PDF ' + pdfDocument.fingerprint + ' [' + info.PDFFormatVersion + ' ' + (info.Producer || '-').trim() + ' / ' + (info.Creator || '-').trim() + '] (PDF.js: ' + (version || '-') + (!PDFJS.disableWebGL
          ? ' [WebGL]'
          : '') + ')');
        ...
        if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('FIREFOX || MOZCENTRAL')) {
          ...
          this
            .externalServices
            .reportTelemetry({type: 'documentInfo', version: versionId, generator: generatorId, formType});
        }
      });
  },
  ...
  setInitialView(storedHash, {rotation, sidebarView} = {}) {
    ...
    // Ensure that the correct page number is displayed in the UI, even if the
    // active page didn't change during document load.
    this
      .toolbar
      .setPageNumber(this.pdfViewer.currentPageNumber, this.pdfViewer.currentPageLabel);
    this
      .secondaryToolbar
      .setPageNumber(this.pdfViewer.currentPageNumber);
  ...
  beforePrint() {
    if (this.printService) {
      // There is no way to suppress beforePrint/afterPrint events, but
      // PDFPrintService may generate double events -- this will ignore the second
      // event that will be coming from native window.print().
      return;
    }
    ...
    // The beforePrint is a sync method and we need to know layout before returning
    // from this method. Ensure that we can get sizes of the pages.
    ...
    let pagesOverview = this
      .pdfViewer
      .getPagesOverview();
    let printContainer = this.appConfig.printContainer;
    let printService = PDFPrintServiceFactory
      .instance
      .createPrintService(this.pdfDocument, pagesOverview, printContainer, this.l10n);
  ...
  rotatePages(delta) {
    ...
    // Note that the thumbnail viewer is updated, and rendering is triggered, in the
    // 'rotationchanging' event handler.
  },
  ...
  bindEvents() {
    ...
    _boundEvents.beforePrint = this
      .beforePrint
      .bind(this);
    _boundEvents.afterPrint = this
      .afterPrint
      .bind(this);
...
if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
  ...
  validateFileURL = function validateFileURL(file) {
    ...
    try {
      ...
      // Removing of the following line will not guarantee that the viewer will start
      // accepting URLs from foreign origin -- CORS headers on the remote server must
  ...
  if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
    ...
    fileInput
      .addEventListener('change', function (evt) {
        ...
        PDFViewerApplication
          .eventBus
          .dispatch('fileinputchange', {fileInput: evt.target});
      });
  } else {
    appConfig
      .toolbar
...
if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
  webViewerOpenFileViaURL = function webViewerOpenFileViaURL(file) {
    if (file && file.lastIndexOf('file:', 0) === 0) {
      // file:-scheme. Load the contents in the main thread because QtWebKit cannot
      // load file:-URLs in a Web Worker. file:-URLs are usually loaded very quickly,
      // so there is no need to set up progress event listeners.
...
function webViewerPageRendered(evt) {
  ...
  let pageView = PDFViewerApplication
    .pdfViewer
    .getPageView(pageIndex);

  // If the page is still visible when it has finished rendering, ensure that the
  // page number input loading indicator is hidden.
  ...
  if (PDFViewerApplication.pdfSidebar.isThumbnailViewVisible) {
    let thumbnailView = PDFViewerApplication
      .pdfThumbnailViewer
      .getThumbnail(pageIndex);
    thumbnailView.setImage(pageView);
  }

  if (PDFJS.pdfBug && typeof Stats !== 'undefined' && Stats.enabled && pageView.stats) {
  ...
  if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('FIREFOX || MOZCENTRAL')) {
    ...
    PDFViewerApplication
      ...
      .then(function (stats) {
        PDFViewerApplication
          .externalServices
          .reportTelemetry({type: 'documentStats', stats});
      });
  }
}
...
function webViewerTextLayerRendered(evt) {
  if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('FIREFOX || MOZCENTRAL') && evt.numTextDivs > 0 && !PDFViewerApplication.supportsDocumentColors) {
    PDFViewerApplication
      .l10n
      .get('document_colors_not_allowed', null, 'PDF documents are not allowed to use their own colors: \'Allow pages to choose t' +
          'heir own colors\' is deactivated in the browser.')
      .then((msg) => {
        console.error(msg);
      });
...
function webViewerNamedAction(evt) {
  // Processing couple of named actions that might be useful. See also
...
function webViewerUpdateViewarea(evt) {
  ...
  let href = PDFViewerApplication
    .pdfLinkService
    .getAnchorUrl(location.pdfOpenParams);
  PDFViewerApplication.appConfig.toolbar.viewBookmark.href = href;
  PDFViewerApplication.appConfig.secondaryToolbar.viewBookmarkButton.href = href;

  // Show/hide the loading indicator in the page number input element.
  let currentPage = PDFViewerApplication
    .pdfViewer
    .getPageView(PDFViewerApplication.page - 1);
...
function webViewerScaleChanging(evt) {
  PDFViewerApplication
    .toolbar
    .setPageScale(evt.presetValue, evt.scale);
...
function webViewerPageChanging(evt) {
  ...
  PDFViewerApplication
    .toolbar
    .setPageNumber(page, evt.pageLabel || null);
  PDFViewerApplication
    .secondaryToolbar
    .setPageNumber(page);
...
function webViewerWheel(evt) {
  ...
  if (evt.ctrlKey || evt.metaKey) {
    ...
    if (previousScale !== currentScale) {
      // After scaling the page via zoomIn/zoomOut, the position of the upper- left
      // corner is restored. When the mouse wheel is used, the position under the
      // cursor should be restored instead.
...
function webViewerKeyDown(evt) {
  ...
  // First, handle the key bindings that are independent whether an input control
  // is selected or not.
  ...
  // Some shortcuts should not get handled if a control/input element is selected.
  let curElement = document.activeElement || document.querySelector(':focus');
  let curElementTagName = curElement && curElement
    .tagName
    .toUpperCase();
  ...
  if (cmd === 0) { // no control key pressed at all.
    switch (evt.keyCode) {
      ...
      case 83: // 's'
        PDFViewerApplication
          .pdfCursorTools
          .switchTool(CursorTool.SELECT);
        break;
      case 72: // 'h'
        PDFViewerApplication
          .pdfCursorTools
          .switchTool(CursorTool.HAND);
        break;
